...
/**
 * Default implementation of ContainerManager using the Docker Java client.
 */
public class DefaultContainerManager implements ContainerManager {
    ...
    public DefaultContainerManager(DockerClient client) {
    ...
    public String createInstance(ContainerConfig config) throws DockerException {
        ...
        hostConfig = hostConfig.withPidsLimit(DockerConstants.DEFAULT_PIDS_LIMIT);
        ...
        try {
            ...
            log.info("Image {} not found locally. Pulling image...", config.image());
            try {
    ...
    private String createContainerInternal(ContainerConfig config, HostConfig hostConfig) {
        ...
        log.debug("Created container instance {}", response.getId());
        return response.getId();
    }

    @Override
    public void startInstance(String id) {
        log.debug("Starting container instance {}", id);
    ...
    public void stopContainer(String id) {
        if (id == null) return;
        try {
            log.debug("Stopping container instance {}", id);
            client.stopContainerCmd(id).exec();
        } catch (Exception e) {
            log.warn("Failed to stop container {}: {}", id, e.getMessage());
        }
    }
    ...
    public void removeContainer(String id) {
        if (id == null) return;
        try {
            log.debug("Removing container instance {}", id);
            client.removeContainerCmd(id).withForce(true).exec();
        } catch (Exception e) {
            log.warn("Failed to remove container {}: {}", id, e.getMessage());
        }
    }
    ...
    public void copyToContainer(String containerId, Path hostPath, Path containerDirectory) throws DockerException {
        try {
            ...
            throw new DockerException("Failed to copy " + hostPath + " to " + containerDirectory, e);
        }
    }
    ...
    public List<String> listFiles(String containerId, String containerPath) {
        try {
            ...
            log.warn("Failed to list files in container {} at {}: {}", containerId, containerPath, e.getMessage());
            return List.of();
        }
    }
    ...
    public void removeFile(String containerId, String containerPath) {
        try {
            ...
            log.warn("Failed to remove file/dir {} in container {}: {}", containerPath, containerId, e.getMessage());
        }
    }
    ...
    public boolean isOOMKilled(String id) {
        try {
            ...
            log.warn("Failed to inspect OOM state for container {}: {}", id, e.getMessage());
        }
        return false;
    }
    ...
    public void kill(String id) {
        ...
        try {
            ...
            log.warn("Failed to kill container {}: {}", id, e.getMessage());
        }
    }

    @Override
    public ExecCMD exec(String id, List<String> command) throws DockerException {
        return execInDir(id, command, null, DockerConstants.DEFAULT_COMPILE_TIMEOUT_SEC);
    }

    @Override
    public ExecCMD execInDir(String id, List<String> command, String workingDir) throws DockerException {
        return execInDir(id, command, workingDir, DockerConstants.DEFAULT_COMPILE_TIMEOUT_SEC);
    }
    ...
    public ExecCMD execInDir(String id, List<String> command, String workingDir, long timeoutSeconds) throws DockerException {
                ...
                if (frame != null && frame.getPayload() != null) {
                    try {
                        ...
                        switch (frame.getStreamType()) {
                            case STDOUT:
                            case RAW:
                                if (stdout.size() < DockerConstants.MAX_STREAM_BYTES) {
                                    int toWrite = Math.min(payload.length, DockerConstants.MAX_STREAM_BYTES - stdout.size());
                                ...
                                break;
                            case STDERR:
                                if (stderr.size() < DockerConstants.MAX_STREAM_BYTES) {
                                    int toWrite = Math.min(payload.length, DockerConstants.MAX_STREAM_BYTES - stderr.size());
        ...
        try {
            ...
            Thread.currentThread().interrupt();
            throw new DockerException("Interrupted while waiting for command to finish.", e);
